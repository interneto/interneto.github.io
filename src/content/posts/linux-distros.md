...
How the five family bases differ across the components that define a distribution. Columns are ordered left to right from the most to the least decisive when choosing one, with closely related components grouped together. *Tumbleweed* (rolling) and *Leap* (fixed) are openSUSE's two editions.

| Distro       | Maintainer                            | Release model                          | Kernel (version / mods)                  | Packaging (manager · build · repos)                                            | Init & logging                                 | Bootloader                        | Desktop & display                          | Installer                                | Base & drivers                                                   |
|:-------------|:--------------------------------------|:---------------------------------------|:-----------------------------------------|:-------------------------------------------------------------------------------|:-----------------------------------------------|:----------------------------------|:-------------------------------------------|:-----------------------------------------|:-----------------------------------------------------------------|
| **Arch**     | Community                             | Rolling                                | Latest stable, near-vanilla              | pacman (`.pkg.tar.zst`); PKGBUILD + makepkg; core/extra + AUR                  | systemd; journald                              | GRUB / systemd-boot (choice)      | None (user choice); Wayland / X11 (choice) | archinstall / manual                     | glibc + GNU coreutils; Mesa, NVIDIA manual                       |
| **Debian**   | Community (Debian Project / SPI)      | Fixed / point release (stable)         | Older LTS, lightly patched               | APT + dpkg (`.deb`); `debian/rules` + dpkg-buildpackage; main/contrib/non-free | systemd; journald (+ optional rsyslog)         | GRUB 2                            | GNOME (selectable); Wayland, X11 available | debian-installer / Calamares (live)      | glibc + GNU coreutils; Mesa, non-free firmware (12+)             |
| **Ubuntu**   | Company (Canonical)                   | Point release (6-month + 2-year LTS)   | LTS base + rolling HWE stacks (Canonical)| APT + dpkg (`.deb`) & Snap (snapd); main/universe/restricted/multiverse + PPAs | systemd; journald                              | GRUB 2                            | GNOME (customized); Wayland (default), X11 | Subiquity / Ubiquity                     | glibc + GNU coreutils; Mesa, proprietary via "Additional Drivers"|
| **Fedora**   | Community, sponsored by Red Hat (IBM) | Point release (~6-month, leading-edge) | Very recent, near-vanilla                | DNF + RPM (`.rpm`); `.spec` + Koji/Mock; Fedora repos + COPR                   | systemd; journald                              | GRUB 2 + BLS                      | GNOME (Workstation); Wayland (default)     | Anaconda                                 | glibc + GNU coreutils; Mesa, proprietary via RPM Fusion          |
| **openSUSE** | Community, sponsored by SUSE          | Leap (fixed) / Tumbleweed (rolling)    | Stable (Leap) / latest (TW), SUSE patches| zypper + RPM (`.rpm`); `.spec` + Open Build Service (OBS)                      | systemd; journald                              | GRUB 2                            | KDE Plasma / GNOME (choice); Wayland / X11 | YaST                                     | glibc + GNU coreutils; Mesa, proprietary via community repos     |
| **Gentoo**   | Community (Gentoo Foundation)         | Rolling                                | User-configured (gentoo-sources / vanilla)| Portage / emerge (`ebuild`, source); Portage tree + overlays (GURU)           | OpenRC (default) or systemd; syslog / journald | GRUB common (choice)              | None (user choice); user choice            | Manual (Handbook), no official installer | glibc (or musl) + GNU coreutils; Mesa, proprietary via USE flags |
...
/* Scope the icon-row tweaks to the first table (Distribution Families) so the
   comparison table below is free to wrap its second column. */
.vp-doc table:first-of-type td:nth-child(2) {
  white-space: nowrap;
}
.vp-doc table:first-of-type td:nth-child(2) a {
  margin: 0 4px;
}
</style>
