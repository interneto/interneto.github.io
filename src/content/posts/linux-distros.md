...
How the five family bases differ across the components that define a distribution, ordered from the most to the least decisive when choosing one. *Tumbleweed* (rolling) and *Leap* (fixed) are openSUSE's two editions.

| Element                              | Arch                                 | Debian                                                    | Fedora                                       | openSUSE                                  | Gentoo                                     |
|:-------------------------------------|:-------------------------------------|:----------------------------------------------------------|:---------------------------------------------|:------------------------------------------|:-------------------------------------------|
| **Maintainer** (community / company) | Community                            | Community (Debian Project / SPI)                          | Community, sponsored by Red Hat (IBM)        | Community, sponsored by SUSE              | Community (Gentoo Foundation)              |
| **Release model**                    | Rolling                              | Fixed / point release (stable)                            | Point release (~6-month, leading-edge)       | Leap (fixed) / Tumbleweed (rolling)       | Rolling                                    |
| **Package manager** (tool / format)  | pacman (`.pkg.tar.zst`)              | APT + dpkg (`.deb`)                                       | DNF + RPM (`.rpm`)                           | zypper + RPM (`.rpm`)                     | Portage / emerge (`ebuild`, source)        |
| **Package build & repository**       | PKGBUILD + makepkg; core/extra + AUR | `debian/rules` + dpkg-buildpackage; main/contrib/non-free | `.spec` + Koji/Mock; Fedora repos + COPR     | `.spec` + Open Build Service (OBS)        | `ebuild` + Portage tree + overlays (GURU)  |
| **Init system**                      | systemd                              | systemd                                                   | systemd                                      | systemd                                   | OpenRC (default) or systemd                |
| **Installer framework**              | archinstall / manual                 | debian-installer / Calamares (live)                       | Anaconda                                     | YaST                                      | Manual (Handbook), no official installer   |
| **Desktop environment** (default)    | None (user choice)                   | GNOME (selectable)                                        | GNOME (Workstation)                          | KDE Plasma / GNOME (choice)               | None (user choice)                         |
| **Window system** (display server)   | User choice (Wayland / X11)          | Wayland (GNOME), X11 available                            | Wayland (default)                            | Wayland / X11 (per DE)                    | User choice                                |
| **System libraries & tools**         | glibc + GNU coreutils                | glibc + GNU coreutils                                     | glibc + GNU coreutils                        | glibc + GNU coreutils                     | glibc (or musl profile) + GNU coreutils    |
| **Kernel** (version / mods)          | Latest stable, near-vanilla          | Older LTS, lightly patched                                | Very recent, near-vanilla                    | Stable (Leap) / latest (TW), SUSE patches | User-configured (gentoo-sources / vanilla) |
| **Bootloader**                       | User choice (GRUB / systemd-boot)    | GRUB 2                                                    | GRUB 2 + BLS                                 | GRUB 2                                    | User choice (GRUB common)                  |
| **Graphics drivers**                 | Mesa; proprietary NVIDIA manual      | Mesa (open); non-free firmware incl. (12+)                | Mesa; proprietary via RPM Fusion (3rd-party) | Mesa; proprietary via community repos     | Mesa; proprietary via USE flags            |
| **Journal / logging**                | systemd-journald                     | systemd-journald (+ optional rsyslog)                     | systemd-journald                             | systemd-journald                          | syslog (OpenRC) / journald (systemd)       |
