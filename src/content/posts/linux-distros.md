...
How these six distributions differ across the components that define a distro. Each component is one row, organized into independent groups; components are ordered from the most to the least decisive when choosing a distro. *Tumbleweed* (rolling) and *Leap* (fixed) are openSUSE's two editions.

| Component                 | Arch                                 | Debian                                | Ubuntu                             | Fedora                            | openSUSE                                  | Gentoo                                     |
|:--------------------------|:-------------------------------------|:--------------------------------------|:-----------------------------------|:----------------------------------|:------------------------------------------|:-------------------------------------------|
| **Governance & releases** |                                      |                                       |                                    |                                   |                                           |                                            |
| Maintainer                | Community                            | Community (Debian Project / SPI)      | Company (Canonical)                | Community + Red Hat (IBM)         | Community + SUSE                          | Community (Gentoo Foundation)              |
| Release model             | Rolling                              | Fixed / point (stable)                | Point (6-month + 2-yr LTS)         | Point (~6-month, leading-edge)    | Leap (fixed) / Tumbleweed (rolling)       | Rolling                                    |
| **Packaging**             |                                      |                                       |                                    |                                   |                                           |                                            |
| Package manager           | pacman (`.pkg.tar.zst`)              | APT + dpkg (`.deb`)                   | APT + dpkg (`.deb`) & Snap         | DNF + RPM (`.rpm`)                | zypper + RPM (`.rpm`)                     | Portage / emerge (`ebuild`, source)        |
| Build & repository        | PKGBUILD + makepkg; core/extra + AUR | `debian/rules`; main/contrib/non-free | dpkg-buildpackage; universe + PPAs | `.spec` + Koji/Mock; repos + COPR | `.spec` + Open Build Service (OBS)        | `ebuild` + Portage tree + overlays (GURU)  |
| **Desktop & install**     |                                      |                                       |                                    |                                   |                                           |                                            |
| Desktop environment       | None (user choice)                   | GNOME (selectable)                    | GNOME (customized)                 | GNOME (Workstation)               | KDE Plasma / GNOME (choice)               | None (user choice)                         |
| Installer framework       | archinstall / manual                 | debian-installer / Calamares          | Subiquity / Ubiquity               | Anaconda                          | YaST                                      | Manual (Handbook), none official           |
| **Graphics & display**    |                                      |                                       |                                    |                                   |                                           |                                            |
| Graphics drivers          | Mesa; NVIDIA manual                  | Mesa; non-free firmware (12+)         | Mesa; "Additional Drivers" tool    | Mesa; via RPM Fusion (3rd-party)  | Mesa; via community repos                 | Mesa; via USE flags                        |
| Window system             | Wayland / X11 (choice)               | Wayland (GNOME), X11 available        | Wayland (default), X11 available   | Wayland (default)                 | Wayland / X11 (per DE)                    | User choice                                |
| **Init & logging**        |                                      |                                       |                                    |                                   |                                           |                                            |
| Init system               | systemd                              | systemd                               | systemd                            | systemd                           | systemd                                   | OpenRC (default) or systemd                |
| Journal / logging         | systemd-journald                     | journald (+ optional rsyslog)         | systemd-journald                   | systemd-journald                  | systemd-journald                          | syslog (OpenRC) / journald (systemd)       |
| **Kernel & boot**         |                                      |                                       |                                    |                                   |                                           |                                            |
| Kernel (version / mods)   | Latest stable, near-vanilla          | Older LTS, lightly patched            | LTS + rolling HWE (Canonical)      | Very recent, near-vanilla         | Stable (Leap) / latest (TW), SUSE patches | User-configured (gentoo-sources / vanilla) |
| Bootloader                | GRUB / systemd-boot (choice)         | GRUB 2                                | GRUB 2                             | GRUB 2 + BLS                      | GRUB 2                                    | GRUB common (choice)                       |
| **Base userland**         |                                      |                                       |                                    |                                   |                                           |                                            |
| System libraries & tools  | glibc + GNU coreutils                | glibc + GNU coreutils                 | glibc + GNU coreutils              | glibc + GNU coreutils             | glibc + GNU coreutils                     | glibc (or musl) + GNU coreutils            |
