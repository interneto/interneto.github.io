...
- **`Society > Portfolio` — reviewed, 2026-09-22.** 55 collections, single clean axis (person's profession/role). `Philarmonic & Orchestra` → `Philharmonic & Orchestra` typo. One real misclassification: `Dr` (a bucket of notable people who go by the honorific "Dr.", mixing real physicians, alt-medicine figures and non-doctors) had Tim Berners-Lee — not a doctor, inventor of the WWW — moved to the existing `Inventor` sibling. Rest of `Dr`'s 13 remaining items don't cleanly decompose into any existing sibling (too mixed: physicians, fringe-medicine authors, and at least one more honorific-only entry, Jimmy Wales, with no good existing home) — left as-is rather than force a fit. No other typos in the branch.
